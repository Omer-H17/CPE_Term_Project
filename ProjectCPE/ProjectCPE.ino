...
unsigned long ledTimes[] = {1100, 1100, 1100, 1100, 1100, 1100, 1100, 1100, 1100, 1100};
...
void meditationAssistanceLoop() {
  ...
  if (pulseSensor.sawStartOfBeat()) {
    ...
    if (bpm < relaxedThreshold) {
      ledFadeToBeat(0,255,0);
    } else if (bpm >= relaxedThreshold && bpm <= moderateThreshold) {
      ledFadeToBeat(0,0,255);  // Moderate BPM indicates moderate stress
    } else {
      ledFadeToBeat(255,0,0);  // High BPM indicates high stress
    }
...
void ledFadeToBeat(int r, int g, int b) {
    // Gradually reduce fade rate for pulse indicator LEDs
  // Apply fade effect to pulse LEDs
  CircuitPlayground.strip.setPixelColor(0, fadeRate * r / 255*BRIGHTNESS, fadeRate * g / 255*BRIGHTNESS, fadeRate * b / 255*BRIGHTNESS);
  CircuitPlayground.strip.setPixelColor(9, fadeRate * r / 255*BRIGHTNESS, fadeRate * g / 255*BRIGHTNESS, fadeRate * b / 255*BRIGHTNESS);

}

int findMax(int num1, int num2, int num3) {
    int maxNum = num1; // Assume num1 is the largest initially

    if (num2 > maxNum) {
        maxNum = num2; // Update if num2 is larger
    }
    if (num3 > maxNum) {
        maxNum = num3; // Update if num3 is larger
    }
